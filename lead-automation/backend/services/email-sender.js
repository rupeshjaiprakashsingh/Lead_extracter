...
async function getSmtpConfig(companyId) {
    if (!companyId) return {};
    const settings = await Settings.find({ companyId, key: { $in: ['smtp_host','smtp_port','smtp_secure','smtp_user','smtp_pass','smtp_from'] } });
    const cfg = {};
...
async function sendEmail(to, subject, html, companyId) {
    const cfg = await getSmtpConfig(companyId);
    logger.log(`Attempting to send email to: ${to} (Subject: "${subject}") for company: ${companyId}`, 'EMAIL');
...
async function testSmtp(companyId) {
    const cfg = await getSmtpConfig(companyId);
    logger.log(`Testing SMTP connection for company: ${companyId} (user: ${cfg.user || 'none'})`, 'SMTP_TEST');
    if (!cfg.user || !cfg.pass) {
